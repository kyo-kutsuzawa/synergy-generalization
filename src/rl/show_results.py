...
def main():
    ...
    parser.add_argument('--filename', type=str, default='result/resultA1.pickle', help='Evaluation results')
    parser.add_argument('--visualize', '-v', nargs='+', choices=['all', 'pos', 'pos2d', 'pos3d', 'err', 'video'], default='all')
    ...
    if 'vel' in args.visualize or 'all' in args.visualize:
        plot_hand_velocity(data, args.out)
...
def plot_hand_positions2d(data, out_dir):
    ...
    # Save the figure
    figname = "trajectories_original.pdf"
...
def plot_hand_positions3d(data, out_dir):
    ...
    # Save the figure
    figname = "trajectories3d_original.pdf"
    fig.savefig(os.path.join(out_dir, figname))
